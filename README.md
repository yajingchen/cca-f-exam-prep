...
│   └── 024_claude-code_app-starter-TDD/ # TDD variant: document & math tools with pytest suite (Python, uv)
│
└── Claude Code in Action/             # Claude Code workflows and agentic patterns
    ...
    └── 002_queries/                   # Natural language queries (TypeScript, Claude Agent SDK, SQLite)
        ├── hooks/                     # PostToolUse hooks: query dedup check, read guard, tsc lint
        └── scripts/                   # init-claude.js: scaffolds .claude/settings.json with hooks
...
# TDD variant — includes document/math tools and a pytest suite
