...
│   ├── 024_claude-code_app-starter/    # Claude Code app starter with MCP server (Python, uv)
│   └── 024_claude-code_app-starter-TDD/ # TDD variant: document tools MCP server + pytest (Python, uv)
│
└── Claude Code in Action/             # Claude Code workflows and agentic patterns
    ├── 000_Video Course Notes – Claude Code in Action.md
    ├── 001_uigen/                     # AI UI generator (Next.js, @ai-sdk/anthropic, Prisma, auth)
...
```bash
# 001_uigen: installs deps, generates Prisma client, and runs migrations
