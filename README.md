...
│   └── 024_claude-code_app-starter-TDD/ # Same project rebuilt via TDD (Python, uv)
│
└── Claude Code in Action/             # Claude Code workflows and agentic patterns
    ├── 000_Video Course Notes – Claude Code in Action.md
    ├── 001_uigen/                     # AI UI generator (Next.js, @ai-sdk/anthropic, Prisma/SQLite, auth)
    └── 002_queries/                   # E-commerce NL query engine (TypeScript, Claude Agent SDK, SQLite, Slack alerts, Claude Code hooks)
...
uv venv --python 3.13 && source .venv/bin/activate && uv pip install -e .
